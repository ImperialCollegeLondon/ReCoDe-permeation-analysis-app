...
class App(ctk.CTk):
    def __init__(self, data_dir):
        ...
        file_label = ctk.CTkLabel(
            self.input_frame, text="Select file:", font=ctk.CTkFont(weight="bold")
        )
        file_label.grid(row=0, column=0, padx=5, pady=5, sticky="w")

        self.file_combobox = ctk.CTkComboBox(
            self.input_frame,
            values=self.get_xlxs_files(),
            width=200,
            command=self.on_combobox_selected,
        )
        self.file_combobox.set("")  # Empty default value
        self.file_combobox.grid(row=0, column=1, padx=5, pady=5, sticky="w")

        # Diameter input
        self.d_cm_label = ctk.CTkLabel(
            self.input_frame, text="Diameter / cm:", font=ctk.CTkFont(weight="bold")
        )
        ...
        # Thickness input
        self.L_cm_label = ctk.CTkLabel(
            self.input_frame, text="Thickness / cm:", font=ctk.CTkFont(weight="bold")
        )
        ...
        # Flow rate input
        self.qN2_mlmin_label = ctk.CTkLabel(
            self.input_frame,
            text="Flow rate / ml min^-1:",
        ...
        self.stab_time_label = ctk.CTkLabel(
            self.input_frame,
            text="Stabilisation time / s:",
            font=ctk.CTkFont(weight="bold"),
        )
        ...
        self.auto_detect_frame.grid(
            row=4, column=1, columnspan=4, padx=5, pady=5, sticky="w"
        )

        self.checkbox_var = ctk.IntVar(value=1)  # Set the default value to 1 (checked)
        self.use_custom_stab_time_checkbox = ctk.CTkCheckBox(
            self.auto_detect_frame,
            text="Auto detect",
            variable=self.checkbox_var,
            command=self.toggle_custom_stab_time_entries,
        )
        ...
        self.tooltip.overrideredirect(
            True
        )  # Hide window decorations to look like pop-up
        self.tooltip_label = ctk.CTkLabel(
            self.tooltip,
            text="Auto detection finds stabilisation time when signal variations fall below threshold.",
            wraplength=200,
        )
        ...
        # Custom time input
        self.custom_stab_time_frame = ctk.CTkFrame(
            self.input_frame, fg_color="transparent"
        )
        self.custom_stab_time_frame.grid(
            row=5, column=1, columnspan=4, padx=5, pady=5, sticky="w"
        )

        self.custom_stab_time_label = ctk.CTkLabel(
            self.custom_stab_time_frame, text="Custom range:"
        )
        self.custom_stab_time_label.pack(side="left", padx=5)

        self.stab_time_start_entry = ctk.CTkEntry(
            self.custom_stab_time_frame, placeholder_text="Start time / s", width=100
        )
        self.stab_time_start_entry.pack(side="left", padx=5)

        self.custom_stab_time_text = ctk.CTkLabel(
            self.custom_stab_time_frame, text=" → "
        )
        self.custom_stab_time_text.pack(side="left", padx=5)

        self.stab_time_end_entry = ctk.CTkEntry(
            self.custom_stab_time_frame, placeholder_text="End time / s", width=100
        )
        ...
        self.run_button = ctk.CTkButton(
            self.input_frame, text="Run Analysis", command=self.run_analysis
        )
        ...
        self.result_text.grid(
            row=7, column=0, columnspan=6, sticky="nsew", padx=10, pady=10
        )
        self.input_frame.grid_rowconfigure(
            7, weight=1
        )  # Adjust row input_frame grid to make textbox expandable
        ...
        self.label_scaling_combobox = ctk.CTkComboBox(
            self.input_frame,
            values=["80%", "90%", "100%", "110%", "120%", "130%", "140%"],
            width=100,
            command=self.change_label_scaling,
        )
        self.label_scaling_combobox.set("100%")  # Default value
        self.label_scaling_factor = (
            int(self.label_scaling_combobox.get().replace("%", "")) / 100
        )
    ...
    def toggle_custom_stab_time_entries(self):
        ...
        if self.checkbox_var.get() == 1:  # ticked, Auto enabled
            self.stab_time_start_entry.configure(
                state="disabled", fg_color="gray", text_color="gray"
            )
            self.stab_time_end_entry.configure(
                state="disabled", fg_color="grey", text_color="grey"
            )
            ...
            self.stab_time_start_entry.configure(
                state="normal", fg_color=default_fg_color, text_color=default_text_color
            )
            self.stab_time_end_entry.configure(
                state="normal", fg_color=default_fg_color, text_color=default_text_color
            )
            self.custom_stab_time_label.configure(
                state="normal", text_color=default_text_color
            )
            self.custom_stab_time_text.configure(
                state="normal", text_color=default_text_color
            )
    ...
    def perform_calculations(self):
        ...
        try:
            ...
            self.result_text.insert(
                ctk.END,
                "Please enter valid numbers for diameter, thickness and flow rate",
            )
            return
        ...
        if self.checkbox_var.get() == 0:
            try:
                self.left_time = (
                    float(self.stab_time_start_entry.get())
                    if self.stab_time_start_entry.get()
                    else None
                )
                self.right_time = (
                    float(self.stab_time_end_entry.get())
                    if self.stab_time_end_entry.get()
                    else None
                )
            except ValueError:
                self.result_text.delete(1.0, ctk.END)
                self.result_text.insert(
                    ctk.END, "Please enter valid numbers for time range"
                )
                return
    ...
    def update_plots(self):
        ...
        result_dict, preprocessed_df, C_profile, flux, df_C, df_flux = (
        ...
        def create_plot_with_save_button(fig, row, column):
            ...
            save_button.bind(
                "<Enter>", lambda e: save_button.configure(font=("", 8, "bold"))
            )
            save_button.bind(
                "<Leave>", lambda e: save_button.configure(font=("", 8, "normal"))
            )
        ...
        plot_time_lag_analysis(
            preprocessed_df,
            result_dict["stabilisation_time"],
            result_dict["slope"],
        ...
        plot_flux_over_time(
            flux, preprocessed_df, preprocessed_df["t / s"].iloc[-1], fig=fig2, ax=ax2
        )
        ...
        plot_concentration_location_profile(
            C_profile, self.L_cm, result_dict["stabilisation_time"], fig=fig3, ax=ax3
        )
        ...
        plot_concentration_profile(
            C_profile, self.L_cm, result_dict["stabilisation_time"], fig=fig4, ax=ax4
        )
