...
def time_lag_analysis(
    df: pd.DataFrame, stabilisation_time_s: float, thickness: float
) -> tuple:
    ...
    if "cumulative flux / cm^3(STP) cm^-2" not in df.columns:
        raise ValueError(
            "cumulative flux / cm^3 cm^-2' does not exist. Please preprocess the data first."
        )
    ...
    slope, intercept = np.polyfit(
        df_ss["t / s"], df_ss["cumulative flux / cm^3(STP) cm^-2"], 1
    )
    ...
    permeability = (
        thickness * steady_state_flux / pressure
    )  # [cm^3(STP) cm^-1 s^-1 bar^-1]
    ...
    solubility_coefficient = (
        permeability / diffusion_coefficient
    )  # [cm^3(STP) cm^-3 bar^-1]
...
def solve_constant_diffusivity_model(
    ...
    df_C_surface, df_flux_values = _create_dataframes(
        C_surface, flux_values, sol, x_grid
    )
...
def _solve_diffusion_pde(
    diffusion_coeff: float, C_eq: float, L: float, T: float, dx: float, dt: float
) -> tuple:
...
def _diffusion_ode(
    t: float, C: np.ndarray, diffusion_coeff: float, dx: float
) -> np.ndarray:
...
def _calculate_flux(
    diffusion_coeff: float, C_surface: np.ndarray, dx: float
) -> np.ndarray:
...
def _create_dataframes(
    C_surface: np.ndarray, flux_values: np.ndarray, sol: Any, x_grid: np.ndarray
) -> tuple:
    ...
    df_C_surface = df_C_surface[
        ["Time"] + [col for col in df_C_surface.columns if col != "Time"]
    ]

    # Flux values DataFrame
    df_flux_values = pd.DataFrame({"Time": sol.t, "Flux": flux_values})
