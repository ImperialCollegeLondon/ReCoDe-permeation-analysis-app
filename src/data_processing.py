...
def load_data(file_path: str) -> pd.DataFrame:
    ...
    if file_path.endswith(".csv"):
        ...
        raise ValueError(
            "Unsupported file format. Please provide a .csv, .xlxs or .xls file."
        )
...
def calculate_flux(
    ...
    if unit == "cm^3 cm^-2 s^-1" or unit == "None":
        df["flux / cm^3(STP) cm^-2 s^-1"] = (
            (df["qN2 / ml min^-1"] / 60) * (df["y_CO2_bl / ppm"] * 1e-6) / A_cm2
        )
...
def calculate_cumulative_flux(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    df["cumulative flux / cm^3(STP) cm^-2"] = (
        df["flux / cm^3(STP) cm^-2 s^-1"] * df["t / s"].diff().fillna(0)
    ).cumsum()
    return df["cumulative flux / cm^3(STP) cm^-2"]


def identify_stabilisation_time(
...
def preprocess_data(
    df: pd.DataFrame, d_cm: float, qN2_mlmin: float = None
) -> pd.DataFrame:
    ...
    # Baseline correction
    baseline_yCO2 = df.loc[
        :10, "y_CO2 / ppm"
    ].mean()  # Baseline is the average of the first 10 data points
    ...
    # Calculate flux
    df["flux / cm^3(STP) cm^-2 s^-1"] = calculate_flux(
        df, d_cm=d_cm, qN2_mlmin=qN2_mlmin, unit="cm^3 cm^-2 s^-1"
    )  # Example conversion factor, thickness, and flow rate
