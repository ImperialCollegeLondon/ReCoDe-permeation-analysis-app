...
def time_lag_analysis_workflow(
    ...
    L_cm: float,
    d_cm: float,
    qN2_mlmin: float = None,
    stablisation_time_range: tuple = (None, None),
    ...
    if (
        stablisation_time_range[0] is not None
        and stablisation_time_range[1] is not None
    ):
        if stablisation_time_range[0] >= stablisation_time_range[1]:
            raise ValueError(
                "The first element of stablisation_time_range should be less than the second element."
            )
    ...
    if stablisation_time_range[0] is not None:
        ...
        stabilisation_time = identify_stabilisation_time(
            df=preprocessed_df,
            column="cumulative flux / cm^3(STP) cm^-2",
            window=70,
            threshold=0.003,
        )
    ...
    stabilisation_index = preprocessed_df.loc[
        preprocessed_df["t / s"] >= stabilisation_time
    ].index[0]
    ...
    df_ss = preprocessed_df.loc[
        (preprocessed_df["t / s"] > stabilisation_time)
        & (preprocessed_df["t / s"] < max_time)
    ]
    ...
    preprocessed_df["normalised flux"] = (
        preprocessed_df["flux / cm^3(STP) cm^-2 s^-1"] / flux_ss
    )
    ...
    (
        ...
        slope,
        intercept,
    ) = time_lag_analysis(preprocessed_df, stabilisation_time, L_cm)

    # Get average temperature
    temperature = preprocessed_df.loc[
        preprocessed_df.index > stabilisation_index, "T / °C"
    ].mean()
    ...
    print(
        f"Solubility Coefficient: {solubility_coefficient:.3g} cm^3(STP) cm^-3 bar^-1"
    )
    ...
    C_profile, flux, df_C, df_flux = solve_constant_diffusivity_model(
    ...
    if save_data:
        try:
            preprocessed_df.to_csv(
                f"{output_dir}/{base_name}_preprocessed_data.csv", index=False
            )
            results_df.to_csv(
                f"{output_dir}/{base_name}_time_lag_analysis.csv", index=False
            )
            df_C.to_csv(
                f"{output_dir}/{base_name}_concentration_profile.csv", index=False
            )
...
if __name__ == "__main__":
    ...
    excel_abs_paths = [
        os.path.abspath(os.path.join(base_dir, "../data", f"{exp_name}.xlsx"))
    ...
    for i, excel_abs_path in enumerate(excel_abs_paths):
        ...
        results = time_lag_analysis_workflow(
            datapath=excel_abs_path,
            L_cm=thickness_dict[exp_names[i]],
            d_cm=1.0,
            qN2_mlmin=qN2_dict[exp_names[i]],
