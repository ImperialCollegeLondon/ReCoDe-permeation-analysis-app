...
def update_ticks(ax, x_lo=None, y_lo=None, x_up=None, y_up=None):
    ...
    if x_up is None:
        if max_x_lim > max_x_tick:
            ax.set_xlim(right=max_x_tick + 1.0 * ax_x_major_tick_length)
    if y_up is None:
        if max_y_lim > max_y_tick:
            ax.set_ylim(top=max_y_tick + 1.0 * ax_y_major_tick_length)
    if x_lo is None:
        if min_x_lim < min_x_tick:
            ax.set_xlim(left=min_x_tick - 1.0 * ax_x_major_tick_length)
    if y_lo is None:
        if min_y_lim < min_y_tick:
            ax.set_ylim(bottom=min_y_tick - 1.0 * ax_y_major_tick_length)
