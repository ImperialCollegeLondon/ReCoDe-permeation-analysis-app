...
def plot_time_lag_analysis(
    df: pd.DataFrame,
    stabilisation_time_s: float,
    slope: float,
    intercept: float,
    fig=None,
    ax=None,
    ...
    ax.plot(
        df_ss["t / s"],
        slope * df_ss["t / s"] + intercept,
        color="red",
        linestyle="--",
        label="Fit (steady-state)",
    )
    ax.plot(
        df.loc[df["t / s"] <= stabilisation_time_s, "t / s"],
        slope * df.loc[df["t / s"] <= stabilisation_time_s, "t / s"] + intercept,
        color="red",
        linestyle=":",
...
def plot_concentration_location_profile(C_profile, L, T, fig=None, ax=None):
    ...
    for t in time_points:
        ax.plot(
            np.linspace(0, L, C_profile.shape[1]),
            C_profile[int(t / T * (C_profile.shape[0] - 1)), :],
            label=f"t = {t:.0f} s",
        )
...
def plot_flux_over_time(flux, preprocessed_df, T_final, fig=None, ax=None):
    ...
    ax.plot(
        preprocessed_df["t / s"],
        preprocessed_df["flux / cm^3(STP) cm^-2 s^-1"],
        linestyle="--",
        label="Measurement",
    )
    ax.set_xlabel(r"Time / $s$")
...
def plot_concentration_profile(C_profile, L, T, fig=None, ax=None):
    ...
    if fig is None or ax is None:
        fig, ax = plt.subplots(1, 1, figsize=figsize_dict["default"])
    cax = ax.imshow(
        C_profile, extent=[0, L, 0, T], aspect="auto", origin="lower", cmap="coolwarm"
    )
    cbar = fig.colorbar(cax, ax=ax)
    cbar.set_label(
        r"Concentration / $cm^{3}(STP) \; cm^{-3}$", size=ax.xaxis.label.get_fontsize()
    )
    ax.set_xlabel(r"Position / $cm$")
